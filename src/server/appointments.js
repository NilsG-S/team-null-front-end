...
export function getUncompAppsByPatient(uid, month) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `${url}appointments/patient/uncompleted/${uid}/${month}`);
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        const map = new Map();
        let date;
...
export function getAppsByPatient(uid) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `${url}appointments/patient/${uid}`);
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        const map = new Map();
        let date;
